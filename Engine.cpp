        ...
        if (! hulp->m_blDestroyed)
		{
			...
			if (hulp->m_blCanCollide)
			{
				...
				while (q!=m_lstItems.end())
				{
					...
					if ((hulp2!=hulp)&&(!hulp->m_blDestroyed)&&(!hulp2->m_blDestroyed)&&((hulp2->m_ID >= ID_ASTEROID)||(hulp2->m_ID == ID_BLACK_HOLE)))
					{
						// Check collision !!!
...
TSprite * TEngine::Seek(int a_nID, double a_dViewDistance, double a_dX, double a_dY)
{
	...
	{
		TSprite*hulp=*p;
		if ((hulp->m_ID==a_nID) && (hulp->m_blDestroyed==false) && (((hulp->m_ID<ID_SHIP_BOTTOM)&&(hulp->m_ID>ID_BASE_TOP))||(((TShip *) hulp)->m_CloakState != CS_CLOAKED)))
		{
