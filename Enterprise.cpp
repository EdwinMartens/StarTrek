...
TEnterprise::TEnterprise()
{
    ...
    // for testing
    /*
    CStatement * pStatement = new CStatement(0);
    pStatement->AddText("This is a test statement,");
    pStatement->AddText("Just for testing the dialog engine");
    pStatement->AddText("Checking it out");

    CAnswer * pAnswer = new CAnswer(-1,0);
    pAnswer->SetText("Testing answer 1");
    pStatement->AddAnswer(pAnswer);

    pAnswer = new CAnswer(-1,0);
    pAnswer->SetText("Testing answer 2");
    pStatement->AddAnswer(pAnswer);

    pAnswer = new CAnswer(-1,0);
    pAnswer->SetText("Testing answer 3");
    pStatement->AddAnswer(pAnswer);

    m_pDialog->AddStatement(pStatement);
    */
}
...
void TEnterprise::DoEngineering()
{
	...
	if ((m_lstHealth[HLT_WARPCORE]<20)&&(m_nEnergy<=0))
   {
...
{
    if ((m_lstHealth[HLT_WARPCORE]>50) && (m_nEnergy > 2000))
    {
                ...
                if (m_lstHealth[HLT_PHASER]>30)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                             ...
                             {
                                  g_pCommunication->AddMessage(15,CREW_SPOCK," Target destroyed");
                             }
                             else
                             {
...
void TEnterprise::DrawOccupationInfo(const Sector & a_sector,int a_nX, int a_nY, int a_nSizeX, int a_nSizeY)
{
    ...
    switch(a_sector.m_nMember)
    {
        ...
        case MEM_FEDERATION:
             col =  m_pEngine->m_clWHITE;
             al_draw_rectangle(a_nX-4, a_nY-4, a_nX+(a_nSizeX-12), a_nY+(a_nSizeY-12), col,4);
             break;
...
void TEnterprise::MouseMove(const ALLEGRO_MOUSE_EVENT & mouse_event)
{
    switch (g_nScreenMode)
    {
        case MODE_COMMUNICATION:
            ...
            {
                m_pTransporter->ProcessMouseEvent(mouse_event);
            }
...
void TEnterprise::MouseButtonDown(const ALLEGRO_MOUSE_EVENT & mouse_event)
{
     switch (g_nScreenMode)
     {
        ...
        if ((mouse_event.button == 1)&&(mouse_event.y > 610)&&(mouse_event.y < 660))
        {
            int nStartX = m_pEngine->m_nScreenMidX-500;
